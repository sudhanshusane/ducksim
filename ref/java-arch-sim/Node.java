...
/**
 * IP Node (Node class) is the resource that is used in the system for message
 * generation as well as message consumption. Node is connected with a switch
...
public class Node {
	...
	/**
	 * An object of InputVCBuffer class. It represents the incoming physical
	...
	// private InputLinkController iLC ;
	...
	// private OutputLinkController oLC ;
	 ...
	 *            address of the node
	 * @param parent
	 *            parent switch of the node
	 * @param pLink
	 *            the physical link number through which the node is connected
	 *            with the switch
	 * @param vcCount
	 *            the number of virtual channel used for every physical channel
	 * @param clkRateFactor
	...
	/*
	 * if (IConstants.TRACE) { System.out.println("Flit received by node " +
	 * address + " at clock " + IConstants.CUR_CYCLE) ; System.out.println("Data
	...
	/**
	 * This method is one of the core methods ot the Node class. It organizes
	 * the operation of the Node object.
	 * 
	 * <p>
	 ...
	 * channel.</li>
	 * <li>Flit’s lastServiceTime statistical field is updated by current clock
	 * cycle</li>
	 * <li>Flit’s virtual channel number is updated by the number found from
	 * messageVCIndex[].</li>
...
//*************************Line below was commented out ******************************
					...
					/** ** Repeated task: Done in the OutputVCBuffer class *** */
					/*
					 * flit.setLastServiceTimeStamp(curCycle);
	 ...
	 * the switch sent by some other nodes to transfer to this node. The method
	...
	/**
	 * This method is called by the Network class at every cycle. The method
				...
				if (inputBuffer.getBufferData(lastInVCServed).getType() == IConstants.HEADER_FLIT) {
					...
					if (nodeReceivedFlitCounter[flit.getVirtualChannelNo()] == 0) {
						if (curCycle > IConstants.WARM_UP_CYCLE) {
							...
							if (curCycle > IConstants.WARM_UP_CYCLE); /*
								System.out.println("Receive Completed: Src: "
										+ flit.getSource() + " Dest: " + address
										+ " cycle: " + curCycle + " hop: "
										+ (flit.getHopCount()-2) + " genTime: "
		...
		while (packet.isEmpty() == false) {
			...
			if (prevHop != flit.getHopCount()) {
				System.out.println("Hop Mismatch " + prevHop + " , "
						+ flit.getHopCount());
				// System.out.println("Flit " + count + " = " + flit.toString())
				// ;
			}
			count++;
		}
