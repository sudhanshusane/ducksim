...
public class NetworkManager {
	...
	private void loadSet(Vector set) {
		int i; int src = 0; int dest = 0; int nd = 0; int flow_id = -1; 
		ParamDTO parDTO;
		String param, val;
		...
		if (null != set) {
			for (i = 0; i < set.size(); i++) {
				...
				else if (param.equalsIgnoreCase("APP"))
					IConstants.APP_FILE = val;
				else if (param.equalsIgnoreCase("ALGORITHM"))
					IConstants.ALGORITHM = Integer.parseInt(val);							
				else if (param.equalsIgnoreCase("FLOW_SRC")){
						src = Integer.parseInt(val);
						IConstants.SOURCE[src]= true; 
						flow_id++; 
					 }
				else if (param.equalsIgnoreCase("FLOW_DEST")){
						dest = Integer.parseInt(val);
						IConstants.DEST[src]= dest;
					 }
				else if (param.equalsIgnoreCase("FLOW_RATE"))
						IConstants.RATE[src]= Integer.parseInt(val); 
				else if (param.equalsIgnoreCase("FLOW_NODE"))
						nd = Integer.parseInt(val);
				else if (param.equalsIgnoreCase("FLOW_DIR")){
						IConstants.FLOWS[src][dest] = flow_id; 
						IConstants.NEXT_HOP[nd][flow_id]= Integer.parseInt(val);
					 }				
			}
		}
