 ...
 * This type of switch is interconnected with different number of switches/nodes
 ...
 * (Node class, resource of the network). All the resources (IP Nodes) are at
 * level 0.
 * 
 * 
 * @version 1.0
...
public class FatTreeSwitch implements Switch {
	...
	/**
	 * This is an array of FatTreeSwitch used to store the list of reference of
	...
	/**
	 * This is array of Node used to store the list of references of adjacent
	 * nodes. One switch may be connected with multiple Nodes and when any flit
	 * is to send or any status of signal is required then the corresponding
	 * node’s reference is retrieved from the nodeList.
	 */
	 ...
	 * These are array of
	 * 
	 * {@link OutputLinkController} used to store the list of output link
	...
	/**
	 * This is an object of type Router. Implements Router's determineRoute()
	 * method.
	 */
	...
	/**
	 * This is the constructor of the FatTreeSwitch used for butterfly fat tree
	 ...
	 * @param switchIndex
	 *            index of this switch in the global array of switches in the
	 * @see Network#createFatTreeNetwork()
	 */
	...
	public FatTreeSwitch(int address, int switchIndex) {
		...
		if (IConstants.TRACE) {
			try {
				...
				raf.seek(raf.length());
				raf.writeBytes("\nCreated Fat Tree Switch (level,index) = ("
	 ...
	 * input link controller’s setOutPathRequest(curCycle) method is called.
	 * 
	 * @param curCycle
	 ...
	 * <li>Flit is retrieved from source virtual channel.</li>
	 ...
	 * @see Network#moveSwitchTrafficFromInputBufferToOutputBuffer(int)
	 */
	 ...
	 * 
	 * @param adjSwitch
	 *            adjacent Butterfly Fat Tree switch
	 * @param linkNo
	 *            physical link number which connects the switch 'adjSwitch'
	 * @param curCycle
	 *            simulation cycle
	 */
	 ...
	 *            node reference
	 * @param linkNo
	 *            physical link number that connects the node to the switch
	 * @param curCycle
	 *            simulation cycle
	 */
	...
	/**
	 * This method is called by Network in every cycle to transfer flit from
	 * output buffer of the switch to input buffer of adjacent nodes and
	 * switches. For every adjacent nodes forwardFlitToNode(adjNode, linkNo,
	...
	public void moveSwitchOutputBufferToInputBufferOfNodeSwitch(int curCycle) {
		...
		// for all adjacent nodes and switches
		for (i = 0; i < noOfPhysicalLink; i++) {
	 ...
	 * implementation of the corresponding specified in the Switch interface.
	 */
