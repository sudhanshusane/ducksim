/**
 * InputLinkController controls the incoming flits and helps in determining the
...
public class InputLinkController {
	 ...
	 * channel.
	 * <p>
	 * Invoked by the adjacent IP nodes and Switches of the parent switch
		...
		for (i = 0; i < this.numVCCount; i++) {
			if ((null != this.inputBuffer.getBufferData(i))
				...
				if (this.inputBuffer.getBufferData(i).getType() == IConstants.HEADER_FLIT) {
					...
					/* Special Implementation for the OCTAL Network */
					...
					// can not be transferred to output port. Flit is blocked
					// here.
				} else {
