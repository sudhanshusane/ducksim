...
public class ConcreteNodeTraffic extends NodeTraffic {
	...
	public Vector generateMessage(int curCycle, int curMessageCount) {
		...
		if (curMessageCount >= IConstants.MAX_MESSAGE_NUMBER) {
			...
			//System.out.println("Node: "+ address + " blocked msg, msg count: "+ curMessageCount);
			//IConstants.SOURCE_MSGS[address]++; 
			return null;
		}
		
		/*
		if (!IConstants.SOURCE[address]) {
			nextMsgGenTime = curCycle + 1;
			return null;
		}
		*/
		...
		IConstants.SOURCE_MSGS[address]++; 
		IConstants.SOURCE_SSSS[address] += (curCycle - IConstants.SOURCE_LAST[address]); 
		IConstants.SOURCE_LAST[address] = curCycle; 
 ...
 /*
		nextMsgGenTime = (int) (-1 * IConstants.AVG_INTER_ARRIVAL * Math
				...
				+ curCycle + 1;
*/
		nextMsgGenTime = IConstants.RATE[address] + curCycle + 1;
	...
	protected int getDestination() {
		...
		if (IConstants.CURRENT_NET == IConstants.NET_MESH) {
			...
			destination = IConstants.DEST[address]; 
	...
	private int generateDestinationForMeshNetwork(int address) {
		...
		if (IConstants.TRAFFIC_TYPE == IConstants.TRAFFIC_UNIFORM) {
			destination = address;
