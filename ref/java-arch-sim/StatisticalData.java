/**
 * <p>
 * StatisticalData class captures various performance parameters of the
...
public class StatisticalData {
	...
	/**
	 * An array capturing the usage status of the slots of the input buffers for
	 * each switch
	 */
	private long switchInBufferUseStatus[];

	/**
	 * An array capturing the usage status of the slots of the output buffers
	 * for each switch
	 */
	...
	/**
	 * Constructor of the StatisticalData class. Initializes the member
	 * variables.
	 * 
	 * @param netType
	 *            type of the network
	...
	public StatisticalData(int netType) {
		...
		/*
		 * if (IConstants.CURRENT_NET == IConstants.NET_MESH ||
		 * IConstants.CURRENT_NET == IConstants.NET_TORUS) { noOfSwitch =
		 * IConstants.NUMBER_OF_IP_NODE / IConstants.MESH_ADJ_NODE ; } else if
		 * (IConstants.CURRENT_NET == IConstants.NET_FAT_TREE ||
		 * IConstants.CURRENT_NET == IConstants.NET_EX_FAT_TREE) { noOfSwitch =
		 * IConstants.FAT_NUM_SWITCH ; } else if (IConstants.CURRENT_NET ==
		 * IConstants.NET_OCTAL) { noOfSwitch = IConstants.NUMBER_OF_IP_NODE /
	 ...
	 * particular node due to the shortage of internal buffers.
	 * 
	 * @param nodeIndex
	 *            index of the IP node
	 */
	 ...
	 *            index of the IP node
	 * @param delay
	 *            delay of the packet to reach the destination
	 */
	 ...
	 *            index of the switch
	 * @param useCount
	 *            the number of links used to transmit packets in a single cycle
	 */
	...
	/**
	 * Increments the count of usage of input buffers of a switch by an amount
	 * 'useCount'
	 * 
	 * @param switchIndex
	 *            index of the switch
	...
	/**
	 * Increments the count of usage of output buffers of a switch by an amount
	 * 'useCount'
	 * 
	 * @param switchIndex
	 *            index of the switch
	...
	/**
	 * Increments the count of usage of input buffers of a node by an amount
	 * 'useCount'
	 * 
	 * @param nodeIndex
	 *            index of the IP node
	...
	/**
	 * Increments the count of usage of output buffers of a node by an amount
	 * 'useCount'
	 * 
	 * @param nodeIndex
	 *            index of the IP node
	...
	public double getAvgPacketDelay() {
		...
		for (int i = 0; i < IConstants.NUMBER_OF_IP_NODE; i++) {
			...
			// System.out.println("Node: " + i + " .. Sent: "+packetSent[i]);
			// System.out.println("Node: " + i + " .. Recv: "+packetReceived[i]+
		...
		System.out.println("Packets throughput = " + (int)(100.0 * (double) numRecv/numSent) +"%");

		System.out.println("Number of Packets Sent in the same unit = " + sameUnit) ;
	 ...
	 * counts by the the number of packets received by the nodes.
	 * </p>
	 * 
	...
	/**
	 * Returns the number of packets <b>NOT</b> produced by all nodes due to
	...
	public double getNumberOfPacketProduced() {
		...
		for (int i = 0; i < IConstants.NUMBER_OF_IP_NODE; i++) {
			...
			//if(packetProduced[i] > 0) System.out.println("Node: " + i + " .. produced "+ packetProduced[i]);
		}
		return numProd;
	}

	/**
	 * Returns the average number of packets <b>NOT</b> produced by the nodes
	 ...
	 * 
	 * 
	 * @return link utilization of all nodes and switches
	 */
	public double getLinkUtilization() {
		...
		for (int i = 0; i < IConstants.NUMBER_OF_IP_NODE; i++) {
			...
			// System.out.println("Node: "+ i+ " .. link Utilization: " +
			// (double)nodeLinkUseStatus[i]/IConstants.NUM_CYCLE);
		}
	 ...
	 * 
	 * <p>
	 * This is the indication of how much of the input buffers are used in
	 * storing flits out of its maximum possible usage. This is calculated by
	 * dividing the summation of number of input buffer in flit unit used in 
	 * storing flit in every simulation cycle by the multiplication of total 
	 * number of simulation cycle and total amount of input buffer in flit unit.
	 * </p>
	 * 
	 * @return the input buffer utilization of all nodes
	 */
	 ...
	 * 
	 * <p>
	 * This is the indication of how much of the output buffers are used in
	 * storing flits out of its maximum possible usage. This is calculated by
	 * dividing the summation of number of output buffer in flit unit used in 
	 * storing flit in every simulation cycle by the multiplication of total 
	 * number of simulation cycle and total amount of output buffer in flit unit.
	 * </p>
	 * 
	 * @return the output buffer utilization of all nodes
	 */
	 ...
	 */
	// calculate input buffer utilization of Switch
	public double getInputBufferSwitchUtilization() {
		...
		//		* (double) noOfSwitch * (double) IConstants.NUM_CYCLE;
	 ...
	 * This is the indication of how much of the output buffers are used in
	 * storing flits out of its maximum possible usage. This is calculated by
	 * dividing the summation of number of output buffers in flit unit used 
	 * in storing flit in every simulation cycle by the multiplication of 
	 * total number of simulation cycle and total amount of output buffers 
	 * in flit unit.
	 * </p>
	 * 
	 * @return the output buffer utilization of all switches
	 */
	// calculate output buffer utilization of Switch
	 ...
	 * 
	 * <p>
	 * This is the indication of how much of the input and output buffers are
	 ...
	 * in flit unit used in storing flit in every simulation cycle by the 
	 * multiplication of total number of simulation cycle and total amount 
	 * of input and output buffers in flit unit by
	 * </p>
	 * 
	 * @return the buffer utilization of all nodes and switches
	 */
	// calculate all buffer utilization of Node & Switch
	public double getBufferUtilization() {
		...
		//temp2 = ((double) IConstants.CURRENT_VC_COUNT
		//		* (double) IConstants.NUM_FLIT_PER_BUFFER
		//		* ((double) noOfSwitch * (double) numLink + (double) IConstants.NUMBER_OF_IP_NODE) * (double) IConstants.NUM_CYCLE);
	 ...
	 * received in all nodes within the total simulation cycles by the
	 * multiplication of the number of nodes and number of simulation cycles.
	 * </p>
	 * 
	 * <p>
	...
	public double getNetworkNodeThroughput() {
		...
		// int numLink = 0 ;
		for (int i = 0; i < IConstants.NUMBER_OF_IP_NODE; i++) {
			temp1 += packetReceived[i];
		}
		
		for (int i = 0; i < IConstants.NUMBER_OF_IP_NODE; i++) {
			temp2 += packetProduced[i];
		}
		
		return (double) temp1/ (temp2);
