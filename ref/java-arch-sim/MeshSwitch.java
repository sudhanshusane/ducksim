...
/**
 * This switch is used for building the Mesh and Torus 
 * network. This type of switch is interconnected with different number of
 * switches according to its position. For example if the switch is in the
 * corner then two other switches are connected with this switch in Mesh
 * topology. If in the side line other than the corners then three other
 * switches are connected with this switch and if in the middle of the network
 * then four other adjacent switches are connected with this switch. Along with
 * these adjacent switches every switch in the network is connected with one or
 * four adjacent nodes. Connection with four nodes provides better throughput.
 * 
 * @version 1.0
 * 
 */

public class MeshSwitch implements Switch {
	...
	/**
	 * This is the array of MeshSwitch used to store the list of reference of
	 ...
	 * These are array of
	 * 
	 * InputLinkController used to store the list of input link controller
	 * objects instantiated for input communication management with the adjacent
	 * nodes and switches. There exists exactly one input link controller for
	 * every physical link.
	 */
	 ...
	 * These are array of
	 * 
	 * OutputLinkController used to store the list of output link controller
	 * objects instantiated for output communication management with the
	 * adjacent nodes and switches. There exists exactly one output link
	...
	/**
	 * Constructor of the MeshSwitch used for mesh and torus topology. The
	 * parameters imply number of Physical link, number of virtual channel per
	 * physical channel, address of this switch, number of nodes attached with
	 ...
	 * @see Network#createMeshNetwork()
	 */
	public MeshSwitch(int pLink, int vLink, int address, int noOfAdjNode,
		...
		switchList = new MeshSwitch[noOfAdjSwitch];
		createRouter();
	...
	public void createRouter() {
		if (IConstants.NET_MESH == IConstants.CURRENT_NET)
			router = new MeshRouter();
		else if (IConstants.NET_TORUS == IConstants.CURRENT_NET)
			router = new TorusRouter();
	}
	...
	/**
	 * This method is called by adjacent nodes and switches to store the flit
	 * sent to it from output buffer of respective node/switch. The method
	 * implemented by simply calling the corresponding input link controller’s
	 * addInputBufferData() method.
	 * 
	 * @param linkNo
	 *            the input link controller where the flit arrives.
	 * @param flit
	 *            Flit data
	 * @param curCycle
	 ...
	 * 
	 * @param adjSwitch
	 *            adjacent Mesh switch
	 * @param linkNo
	 *            physical link number which connects the switch 'adjSwitch'
	 * @param curCycle
	 *            simulation cycle
	 */
	private void forwardFlitToSwitch(MeshSwitch adjSwitch, int linkNo,
			int curCycle) {
		int count = 0, toLink, apLink, numNode;
		Flit flit;

		numNode = IConstants.MESH_ADJ_NODE;
	...
	private int getMeshSwitchOutAdjLinkNo(int linkNo) {
		if (linkNo == IConstants.SWITCH_LEFT) {
			return IConstants.SWITCH_RIGHT;
		} else if (linkNo == IConstants.SWITCH_TOP) {
			return IConstants.SWITCH_BOTTOM;
		}

		else if (linkNo == IConstants.SWITCH_RIGHT) {
			return IConstants.SWITCH_LEFT;
		}

		else if (linkNo == IConstants.SWITCH_BOTTOM) {
			return IConstants.SWITCH_TOP;
		}
	...
	public void setAdjacentSwitch(Switch meshSwitch, int linkNo) {
		switchList[linkNo] = (MeshSwitch) meshSwitch;
	}
